...
class eppCreateDomainRequest extends eppDomainRequest {
     ...
     * @return void
     */
    public function addLang($tag, $version = 'http://www.twnic.net.tw/epp/idn-1.0')
    {
        if (!$this->extension) {
            $this->extension = $this->createElement('extension');
            $this->getCommand()->appendChild($this->extension);
        }
        $langCreate = $this->createElement('idnLang:tag', $tag);
        $langCreate->setAttribute('xmlns:idnLang', $version);
        $this->extension->appendChild($langCreate);
        $this->addSessionId();
    }
}
